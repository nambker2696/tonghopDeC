...
void menu_tudien();    // show menu
char *nhap_xau();
char *random_s();
void doc_file(BTA *Dic,BTA *Sou,char file_name[]);  // file_name abc.txt, Dic la con tro Btree chua tu dien, Sou chua ki tu ma hoa dc cua tu
void ghi_file(BTA *Dic,char file_name[]);  // ghi du lieu tu Dic vao file_name.txt
void ghi_file_sou(BTA *Sou,char file_name[]); // nhu tren
int add_dic(BTA *Dic,char *w,char *m);  // w: tu, m: nghia
int add_sou(BTA *Sou,char *w);  // w: tu
int add_word(BTA *Dic,BTA *Sou,char *w,char *m); // chuc nang gop 2 ham tren
int tab_complete(BTA *btfile,char *key); // key: tu viet do
mean *search_mean(BTA *Dic,char *w);
void print_mean(mean *m,char *w);
soundex *search_soundex(BTA *Sou,char *w);
...
int add_dic(BTA *Dic,char *w,char *m)
{
  mean *s;
  int rsize,k=0,i;
  s=(mean*)malloc(sizeof(mean));
  if(btsel(Dic,w,(char *)s,sizeof(mean),&rsize)==0)
    {
      for(i=0;i<s->count;i++)
      ...
      else
	{
	  ...
	  s->count++;
	  btupd(Dic,w,(char *)s,sizeof(mean));
	}
    }
  else
    {
      strcpy(s->list[0],m);
      s->count=1;
      if(btins(Dic,w,(char *)s,sizeof(mean))!=0)
	printf("\n Loi.\n");
    }
  free(s);
  return 1;
...
int add_sou(BTA *Sou,char *w)
{
  ...
  soundEx(s,w,50,1);
  if(btsel(Sou,s,(char *)se,sizeof(soundex),&rsize)==0)
    {
      ...
      else
	{
	  ...
	  se->count++;
	  btupd(Sou,s,(char *)se,sizeof(soundex));
	}
    }
  else
    {
      strcpy(se->word[0],w);
      se->count=1;
      btins(Sou,s,(char *)se,sizeof(soundex));
    }
  free(se);
  return 1;
}
...
void doc_file(BTA *Dic,BTA *Sou,char file_name[])
{
  FILE *f;
  ...
  if(f==NULL)
    {
      ...
      printf("\n=====> Chuyen sang tu dong tao tu dien.\n");
      printf("\n  Xin cho de tao du lieu!\n");
      for(i=0;i<MAX;i++)
	{
	  s=random_s();
	  add_word(Dic,Sou,s,s);
	  free(s);
	}
...
void ghi_file(BTA *Dic,char file_name[])
{
  FILE *f;
  BTint in;
  int rsize,i;
  ...
  btpos(Dic,ZSTART);
  while(bnxtky(Dic,tu,&in)==0)
    {
      btsel(Dic,tu,(char *)s,sizeof(mean),&rsize);
      for(i=0;i<s->count;i++)
	fprintf(f,"%s\t%s\n",tu,s->list[i]);
    }
  free(tu);
...
void ghi_file_sou(BTA *Sou,char file_name[])
{
  FILE *f;
  BTint in;
  int rsize,i;
  ...
  btpos(Sou,ZSTART);
  while(bnxtky(Sou,tu,&in)==0)
    {
      btsel(Sou,tu,(char *)s,sizeof(soundex),&rsize);
      for(i=0;i<s->count;i++)
	fprintf(f,"%s\t%s\n",tu,s->word[i]);
    }
  free(tu);
...
int main(int argc,char *argv[])
{
  ...
  if(thay_doi==1)// Neu them hay xoa mot tu thi se cap nhat lai tu dien.
    {
      ghi_file(Dic,argv[1]);
      printf("\n  Xin cho de luu lai du lieu.\n");
      if(strcmp(argv[2],"Dic")==0)
	ghi_file_sou(Sou,"soundex.txt");
      else
