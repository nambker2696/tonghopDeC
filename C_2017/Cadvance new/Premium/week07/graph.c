...
void BFS(graph g, int start, int stop, void (*visited_func)(int)){
     ...
     int i;
     for(i = 0; i < v; i++) visited[i] = 0;  //visited[i] = false

     int output[v];
     
     Dllist queue = new_dllist();            //initialize an empty Q
     


     // add start node to queue    Enqueue()
     ...
     while(!dll_empty(queue)){
          ...
          // }
          n = get_adjacent_vertices(g, u, output);
          for (i = 0; i < n; ++i)
